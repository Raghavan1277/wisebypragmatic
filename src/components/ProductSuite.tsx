              ...
              >
                <div className={`rounded-3xl bg-gradient-to-br ${product.gradient} border ${product.borderColor} hover:shadow-2xl transition-all overflow-hidden`}>
                  <div className={`grid lg:grid-cols-2 gap-0 ${!isEven ? 'lg:grid-flow-dense' : ''}`}>
                    ...
                    {/* Image/Visual Column */}
                    <div className={`relative ${product.title === "Co-Analyst AI" ? "min-h-[200px] lg:min-h-[250px]" : "min-h-[400px] lg:min-h-[500px]"} bg-gradient-to-br ${product.gradient} flex items-center justify-center overflow-hidden ${!isEven ? 'lg:col-start-1' : ''}`}>
                      {product.image ? (
                        <img 
                          src={product.image} 
